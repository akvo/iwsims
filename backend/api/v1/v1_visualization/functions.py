...
from django.db.models import (
    Q, Count, Avg, Subquery, OuterRef, F,
)
...
def apply_administration_filter(queryset, administration_id):
    ...
    adm_path = (
        f"{adm.path}{adm.id}." if adm.path else f"{adm.id}."
    )
...
def latest_monitoring_subquery(form_id, date_filters=None):
    ...
    if date_filters:
        ...
        if date_qid:
            ...
            if date_filters.get("from_date"):
                qs = qs.filter(
                    created__date__gte=date_filters["from_date"]
                )
            if date_filters.get("to_date"):
                qs = qs.filter(
                    created__date__lte=date_filters["to_date"]
                )
...
def get_base_monitoring_qs(form, monitoring_form_id, params):
    ...
    if date_question_id:
        date_filters["date_question_id"] = date_question_id

    is_monitoring = form.parent is not None
    parent_form = form.parent if is_monitoring else form
...
def get_monitoring_data_ids(qs, is_latest_mode):
    ...
    if is_latest_mode:
        return list(qs.values_list("latest_id", flat=True))
