# Task 1: Create Migration File for Materialized Views

## Overview

Create a Django migration that adds 4 new materialized views to optimize the visualization API. These views pre-compute expensive queries that currently run on every API request.

## File to Create

**Path**: `backend/api/v1/v1_visualization/migrations/0002_add_optimized_materialized_views.py`
...
| View | Purpose | Expected Speedup |
|------|---------|------------------|
| `mv_latest_monitoring` | Pre-computes latest monitoring submission per parent, eliminating correlated subquery | 10-50x |
| `mv_answer_denormalized` | Pre-joins answers with data/question tables for faster filtering | 5-20x |
| `mv_cross_form_latest` | Aggregates across ALL monitoring forms by `question_name` | New feature |
| `mv_parent_aggregates` | Pre-aggregated option counts and numeric stats per parent | 5-10x |
...
2. Verify views exist:
   ```bash
   ./dc.sh exec backend python manage.py dbshell
   ```
   ```sql
   \dm+  -- List materialized views
   ```

3. Check row counts (should have data if there's existing form data):
   ```sql
   SELECT 'mv_latest_monitoring' as view, COUNT(*) FROM mv_latest_monitoring
   UNION ALL
   SELECT 'mv_answer_denormalized', COUNT(*) FROM mv_answer_denormalized
   UNION ALL
   SELECT 'mv_cross_form_latest', COUNT(*) FROM mv_cross_form_latest
   UNION ALL
   SELECT 'mv_parent_aggregates', COUNT(*) FROM mv_parent_aggregates;
   ```
